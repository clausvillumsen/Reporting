...
export class ParentFilterComponent extends React.Component<Props, State> {
            ...
            <div className="dropdown part " ref={this.node} >
                ...
                {this.state.popupVisible && (
                    <div className="popover">
                        <FilterComponent onSelectedFilter={(selectedModel: FilterData) => this.handleSelectedFilter(selectedModel)} />
                        <div className="labelHeader">SØG PÅ BRUGER ATTRIBUTER</div>
                        <input placeholder="F.eks. København" type="text" className="searchInput" aria-describedby="basic-addon2" ref={this.searchInput}></input>
                        <div className="bottomButton">
                            <div className="btn btn-default btnCommon btnHeader floatLeft" onClick={() => this.clearFilter()}><span>Clear</span></div>
                            <div className="btn btn-default btnCommon btnHeader floatRight" onClick={() => this.applyFilter()}><span>Tilføj</span></div>
                        </div>
                    </div>
                )}
            </div>
    ...
    handleSelectedFilter = (selectedModel: FilterData) => {
        this.setState(prev => {
            return {
                ...prev, currentFilter: selectedModel
            }
        })
    }
